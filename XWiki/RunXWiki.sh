...
docker run -v ${PWD}/mysql:/volume -v ${PWD}/Backup:/backup --rm kristant/volume-backup restore mysql > /dev/null
...
docker run -v ${PWD}/xwiki:/volume -v ${PWD}/Backup:/backup --rm kristant/volume-backup restore xwiki > /dev/null
